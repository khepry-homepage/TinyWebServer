...
void HttpConn::Process()
{
    ...
    if (!write_ret)
    {
        LOG_ERR("%s %s", m_log_buf, "error: failed in calling ProcessWrite(HTTP_CODE)");
        CloseConn();
    }
    // 注册写事件
...
HttpConn::HttpConn()
{
    ...
    if (hr == nullptr)
    {
        LOG_ERR("%s %s", m_log_buf, "error: failed in creating HttpRequest instance");
        throw std::exception();
    }
...
HttpConn::ProcessRead()
{
    ...
    while (hr->process_state == CHECK_STATE_BODY || (line_state = ParseLine()) == LINE_OK)
    {
        ...
        {
            m_log_buf_idx += snprintf(m_log_buf + m_log_buf_idx, LOG_BUF_SIZE - m_log_buf_idx, "[client %s] %s ", m_clientIP, line);
...
bool HttpConn::ProcessWrite(HTTP_CODE code)
{
    ...
    {
    case FILE_REQUEST:
        m_log_buf_idx += snprintf(m_log_buf + m_log_buf_idx, LOG_BUF_SIZE - m_log_buf_idx, "%d %s", 200, hr->header_option[HttpRequest::User_Agent]);
    ...
    case GET_REQUEST:
        ...
        break;
    case BAD_REQUEST:
        m_log_buf_idx += snprintf(m_log_buf + m_log_buf_idx, LOG_BUF_SIZE - m_log_buf_idx, "%d %s", 400, hr->header_option[HttpRequest::User_Agent]);
    ...
    case FORBIDDEN_REQUEST:
        m_log_buf_idx += snprintf(m_log_buf + m_log_buf_idx, LOG_BUF_SIZE - m_log_buf_idx, "%d %s", 403, hr->header_option[HttpRequest::User_Agent]);
        ...
        break;
    case NO_RESOURCE:
        m_log_buf_idx += snprintf(m_log_buf + m_log_buf_idx, LOG_BUF_SIZE - m_log_buf_idx, "%d %s", 404, hr->header_option[HttpRequest::User_Agent]);
        ...
        break;
    case INTERNAL_ERROR:
        m_log_buf_idx += snprintf(m_log_buf + m_log_buf_idx, LOG_BUF_SIZE - m_log_buf_idx, "%d %s", 500, hr->header_option[HttpRequest::User_Agent]);
...
HttpConn::ParseHeader(char *line, HttpRequest *hr)
{
    ...
    *tmp++ = '\0';
    if (strcmp(line, "Host") == 0)
        hr->header_option.emplace(HttpRequest::Host, tmp);
    else if (strcmp(line, "User-Agent") == 0)
        hr->header_option.emplace(HttpRequest::User_Agent, tmp);
    else if (strcmp(line, "Accept") == 0)
        hr->header_option.emplace(HttpRequest::Accept, tmp);
    else if (strcmp(line, "Referer") == 0)
        hr->header_option.emplace(HttpRequest::Referer, tmp);
    else if (strcmp(line, "Accept-Encoding") == 0)
        hr->header_option.emplace(HttpRequest::Accept_Encoding, tmp);
    else if (strcmp(line, "Accept-Language") == 0)
        hr->header_option.emplace(HttpRequest::Accept_Language, tmp);
    else if (strcmp(line, "Connection") == 0)
        hr->header_option.emplace(HttpRequest::Connection, tmp);
