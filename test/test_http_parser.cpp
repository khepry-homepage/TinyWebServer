...
TEST(HTTP_PARSER_TEST, PROCESS_READ) {
  char text[] =
      ...
      "\r\n";
  char err_text1[128] =
      "GET /562f25980001b1b106000338.jpg HTTP/1.1\r"
      "\nHost: img.mukewang.com\r\n";
  ...
  ASSERT_TRUE(hc.ProcessRead() == HttpConn::NO_REQUEST);
  EXPECT_EQ(string(hr->header_option[HttpRequest::User_Agent]),
            "Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, "
            "like Gecko) Chrome/51.0.2704.106 Safari/537.36");
  hc.CopyReadBuf(text + 192, 38);
  ASSERT_TRUE(hc.ProcessRead() == HttpConn::NO_REQUEST);
  EXPECT_EQ(string(hr->header_option[HttpRequest::Accept]),
            "image/webp,image/*,*/*;q=0.8");
  hc.CopyReadBuf(text + 230, 32);
  ASSERT_TRUE(hc.ProcessRead() == HttpConn::NO_REQUEST);
  EXPECT_EQ(string(hr->header_option[HttpRequest::Referer]),
            "http://www.imooc.com/");
  hc.CopyReadBuf(text + 262, 38);
  ASSERT_TRUE(hc.ProcessRead() == HttpConn::NO_REQUEST);
  EXPECT_EQ(string(hr->header_option[HttpRequest::Accept_Encoding]),
            "gzip, deflate, sdch");
  hc.CopyReadBuf(text + 300, 33);
  ASSERT_TRUE(hc.ProcessRead() == HttpConn::NO_REQUEST);
  EXPECT_EQ(string(hr->header_option[HttpRequest::Accept_Language]),
            "zh-CN,zh;q=0.8");
